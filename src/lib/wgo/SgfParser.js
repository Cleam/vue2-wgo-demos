...
// parse SGF string, return Kifu object
SGF.parse = function(str) {
  ...
  for (var i in sequence) {
    ...
    // reading node (string starting with ';')
      ...
      node = node ? node.appendChild(new KNode()) : kifu.root;
      ...
      for (var j in props) {
        ...
        if (SGF.properties[ident]) SGF.properties[ident](kifu, node, vals, ident);
        else {
